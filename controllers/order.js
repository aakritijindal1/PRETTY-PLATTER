...
exports.addRestaurant = async (req, res) => {
  try {
    ...
    await Restaurant.findByIdAndUpdate(restaurant._id, {
      $push: { menuItemIds: { $each: menuItemIds } },
    });
...
exports.createOrder = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Order placed successfully, waiting for Restaurent to accept it!",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
    ...
    if (status === "accepted") {
      ...
      if (agents.length > 0) {
        ...
        await Order.findByIdAndUpdate(order_id, {
          deliveryAgent: assignedAgent._id,
        });
        await Agents.findByIdAndUpdate(assignedAgent._id, {
          isAvailable: false,
        });
